...
public class XMLFiles {
	...
	public static void decompressXML(byte[] xml) {
		...
		while (off < xml.length) {
			...
			if (tag0 == startTag) { // XML START TAG
				...
				for (int ii = 0; ii < numbAttrs; ii++) {
					...
					String attrValue = attrValueSi != -1 ? compXmlString(xml, sitOff, stOff, attrValueSi)
...
//		prt("    end at offset " + off);
	...
	public static int LEW(byte[] arr, int off) {
		return arr[off + 3] << 24 & 0xff000000 | arr[off + 2] << 16 & 0xff0000 | arr[off + 1] << 8 & 0xff00
				| arr[off] & 0xFF;
	} // end of LEW
